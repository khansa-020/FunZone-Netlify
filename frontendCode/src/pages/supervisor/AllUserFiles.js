...
import LeftSide from "../../components/left/LeftSideSup";
...
const AllUserFiles = () => {
  ...
  const handleGetFiles = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/fileupload/files/${params.id}`
      );
      if (res.data.success) {
  ...
  return (
    <>
      ...
      >
        <p>Are you sure, you want to delete the file permanently?</p>
      </Modal>
    </>
  );
};
